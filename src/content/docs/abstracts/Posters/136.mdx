...
Another big aspect is the support of human-centered metadata authoring and experimental documentation. The ISA software suite, CEDAR, and electronic laboratory notebook approaches such as eLabFTW provide structured forms, templates, ontology-based annotation, validation, and reusable metadata capture for experimental organization and repository submission <Cite id="carpi_elabftw_2017, musen_center_2015, rocca-serra_isa_2010"/>. These systems lower the burden of metadata entry and support standards-compliant annotation, particularly when users need to curate experimental descriptions rather than inspect a large provenance graph.
