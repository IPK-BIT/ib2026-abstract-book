...
<BookmarkButton
  id="/ib2026-abstract-book/abstracts/full-talks/132/"
  title={frontmatter.title}
  tagline={frontmatter.hero?.tagline}
  authors={frontmatter.authors}
/>
...
In this contribution, we present the software library ARCtrl as the central reference implementation of the ARC data model and its representations <Cite id="weil_nfdi4plantsarctrl_2026"/>. ARCtrl provides the core functionality required to parse, manipulate, and convert ARCs across their different representations. It acts as an authoritative software layer between the ARC specification and applications that depend on ARC functionality. This is important because specifications alone cannot prevent divergent interpretations across independently developed tools. By centralizing core ARC behavior in a shared implementation, ARCtrl aims to reduce fragmentation within the ARC community, providing a stable foundation for a wide range of downstream applications.
